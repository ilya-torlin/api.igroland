...
class ExportController extends ActiveController {
     ...
     * )
     */
    public function actionGetupdates($link) {
        $maxLimit = 5000;
        $this->export = \app\models\Export::find()->where(['link' => $link])->one();
        if (!$this->export) {
            return JsonOutputHelper::getError('Экспорт не найден');
        ...
        //TO DO перенести логику в каталог

        $categoryIds = array();
        $depth = 0;
        $models = \app\models\Category::find();
        $models = $models->where(['catalog_id' => $this->export->catalog_id]);
        $newCategoryIds = $models->select('id')->asArray()->column();
        ...
        $models = $models->leftJoin('product_attach', 'product.id = product_attach.attached_product_id');
        $models = $models->leftJoin('product_category', 'product.id = product_category.product_id');
        $models = $models->andWhere(['>', 'timestamp', $params['timestamp']]);
        $models = $models->andWhere(['or',
            ['IN', 'product_attach.category_id', $categoryIds],
            ['IN', 'product_category.category_id', $categoryIds]
        ]);



        $products = $models->orderBy('timestamp')->limit($maxLimit)->all();
        ...
        foreach ($products as $product) {
            ...
            $results[] = array('name' => $product->title,
                'id' => $product->id,
                'articul' => $product->sku,
                'price' => $price,
                'roznPrice' => $roznPrice,
        ...
        $resultsDupl = new \ArrayObject($results);
        $resultsDuplArr = $resultsDupl->getArrayCopy();
        
        if (count($results) == $maxLimit) {
            $num = $maxLimit - 1;
            while ($num > 0 && $results[$num]['timestamp'] == $results[$num - 1]['timestamp'] ) {
                unset($results[$num]);
                $num--;
            }
            if ($num >= 0) unset($results[$num]);
        }
        
        if (count($results) == 0){
            return $resultsDuplArr;
        }
    ...
    public function behaviors() {
        ...
        $behaviors['authenticator']['except'] = ['options', 'view', 'getupdates'];
